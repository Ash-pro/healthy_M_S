...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.customerPayment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.customer-payments.update", [$customerPayment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('patient') ? 'is-invalid' : '' }}" name="patient_id" id="patient_id" required>
                    @foreach($patients as $id => $entry)
                        <option value="{{ $id }}" {{ (old('patient_id') ? old('patient_id') : $customerPayment->patient->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_time') ? 'is-invalid' : '' }}" type="text" name="date_time" id="date_time" value="{{ old('date_time', $customerPayment->date_time) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payments') ? 'is-invalid' : '' }}" type="number" name="payments" id="payments" value="{{ old('payments', $customerPayment->payments) }}" step="0.01" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('doctor_revealed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="doctor_revealed" id="doctor_revealed" value="1" {{ $customerPayment->doctor_revealed || old('doctor_revealed', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('lab_detection') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="lab_detection" id="lab_detection" value="1" {{ $customerPayment->lab_detection || old('lab_detection', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('buy_medicine') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="buy_medicine" id="buy_medicine" value="1" {{ $customerPayment->buy_medicine || old('buy_medicine', 0) === 1 ? 'checked' : '' }}>
