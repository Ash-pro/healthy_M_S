    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.salaries.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('d_name') ? 'is-invalid' : '' }}" name="d_name_id" id="d_name_id" required>
                    @foreach($d_names as $id => $entry)
                        <option value="{{ $id }}" {{ old('d_name_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('d_salary') ? 'is-invalid' : '' }}" type="number" name="d_salary" id="d_salary" value="{{ old('d_salary', '') }}" step="0.01" required>
