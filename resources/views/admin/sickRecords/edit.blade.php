...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.sickRecord.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sick-records.update", [$sickRecord->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('p_name') ? 'is-invalid' : '' }}" name="p_name_id" id="p_name_id" required>
                    @foreach($p_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('p_name_id') ? old('p_name_id') : $sickRecord->p_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('reception_recording') ? 'is-invalid' : '' }}" type="text" name="reception_recording" id="reception_recording" value="{{ old('reception_recording', $sickRecord->reception_recording) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('doctors_diagnosis') ? 'is-invalid' : '' }}" name="doctors_diagnosis" id="doctors_diagnosis">{!! old('doctors_diagnosis', $sickRecord->doctors_diagnosis) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('laboratory_analysis') ? 'is-invalid' : '' }}" name="laboratory_analysis" id="laboratory_analysis">{!! old('laboratory_analysis', $sickRecord->laboratory_analysis) !!}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\SickRecord::RECEIVING_MEDICINE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('receiving_medicine') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="receiving_medicine_{{ $key }}" name="receiving_medicine" value="{{ $key }}" {{ old('receiving_medicine', $sickRecord->receiving_medicine) === (string) $key ? 'checked' : '' }}>
