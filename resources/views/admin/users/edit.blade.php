...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.user.fields.account_type') }}</label>
                <select class="form-control {{ $errors->has('account_type') ? 'is-invalid' : '' }}" name="account_type" id="account_type" required>
                    ...
                    @foreach(App\Models\User::ACCOUNT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('account_type', $user->account_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.account_type_helper') }}</span>
            </div>
            <div class="form-group">
